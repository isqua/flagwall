...
export class CountrySelector {
    ...
    initialize(countries: Country[]) {
        countries.forEach((country) => {
            ...
            querySelectorSafe<HTMLInputElement>("input", clone).value =
                country.code;
            querySelectorSafe<HTMLInputElement>("input", clone).id =
                country.code;
            querySelectorSafe<HTMLLabelElement>("label", clone).setAttribute(
                "for",
                country.code,
            );
            querySelectorSafe<HTMLSpanElement>("span", clone).innerText =
                country.name;
            querySelectorSafe<HTMLImageElement>("img", clone).src = getFlagSrc(
                country.code,
            );
    ...
    setVisibleCountries(codes: Set<string>) {
        const countries =
            this.container.querySelectorAll<HTMLElement>("[data-code]");
